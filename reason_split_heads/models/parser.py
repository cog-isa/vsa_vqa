...
class Seq2seqParser():
    ...
    def supervised_forward(self):
        ...
        func_output_logprob, arg_output_logprob = self.seq2seq(self.x, self.y, self.input_lengths)
        
        func_y = torch.zeros_like(self.y)
        arg_y = torch.zeros_like(self.y)
        
        for i in range(self.y.shape[0]):
            for j in range(self.y.shape[1]):
                func, arg = self.vocab['program_idx_decouple'][str(self.y[i, j].item())]
                func_y[i, j] = func
                arg_y[i, j] = arg

        self.loss = self.criterion(func_output_logprob[:,:-1,:].contiguous().view(-1, func_output_logprob.size(2)), 
                                    func_y[:,1:].contiguous().view(-1))
        self.loss +=  self.criterion(arg_output_logprob[:,:-1,:].contiguous().view(-1, arg_output_logprob.size(2)), 
                                    arg_y[:,1:].contiguous().view(-1))
